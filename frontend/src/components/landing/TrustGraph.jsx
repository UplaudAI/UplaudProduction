  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      nodes.forEach((n) => {
        ...
        ctx.fillStyle = n.hot ? "#10b981" : "rgba(255,255,255,0.55)";
        ctx.fill();
...
export default function TrustGraph() {
  return (
    <section
      ...
      className="relative overflow-hidden bg-[#050505] text-[#fdfdfb] py-28 md:py-40 noise"
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-7">
            <span className="chip chip-dark" data-testid="engine-eyebrow">
              <Sparkles className="w-3.5 h-3.5 text-[#10b981]" strokeWidth={1.75} />
              The AI Trust Graph
            </span>
            <h2
              data-testid="engine-headline"
              className="mt-6 font-display text-[40px] sm:text-[56px] lg:text-[76px] leading-[1.02] font-medium tracking-tight"
            >
              An engine that learns
              <br />
              <span className="text-[#10b981]">which trust</span> actually
              converts.
            </h2>
            <p className="mt-8 max-w-2xl text-[16px] md:text-[18px] leading-relaxed text-white/70">
              Uplaud ingests every review, comment and referral, tags them by
              story, context and intent, and maps the clusters that convert for
              your business. Then it feeds high-signal audiences and creative
              back into your ad accounts — so every dollar rides on warm data.
            </p>
          </div>
...
function StatCard({ testId, label, value, foot, highlight }) {
  return (
    <div
      data-testid={testId}
      className={`relative overflow-hidden border rounded-sm p-5 backdrop-blur-md ${
        highlight
    ...
    >
      ...
      <div
        className={`mt-3 font-display text-[36px] leading-none tracking-tight ${
          highlight ? "text-[#10b981]" : "text-[#fdfdfb]"
        }`}
      >
        {value}
      </div>
